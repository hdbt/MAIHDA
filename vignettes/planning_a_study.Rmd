---
title: "Planning a MAIHDA study"
author: "Hamid Bulut"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Planning a MAIHDA study}
...
Most MAIHDA mistakes are made *before* the model is fit -- in the choice of
dimensions, the number of strata, and the analytic sample. This vignette walks
...
It is *not* a causal method: the VPC, PCV, and discriminatory accuracy are
descriptive variance/prediction summaries, not effects of identity on the outcome.
...
section). A useful discipline: choose the **fewest dimensions that answer your
...
If you see a singular-fit note, do not read the VPC as a clean zero. The remedies
are to **collapse dimensions or categories** (fewer, larger cells), or to use
`engine = "brms"`, whose weakly-informative priors regularise the variance off the
boundary and return an honest interval -- the subject of the
...
- **Keep continuous variables out of the strata.** A continuous variable in the
  ...
  in the fixed part of the formula, not the strata. Set `autobin = FALSE`, or bin it
  yourself for reproducible cut-points.
- **Decide the analytic sample up front.** The VPC and PCV must be compared on the
  *same* rows, so define one complete-case sample before fitting the null and
  adjusted models. For substantial missingness, multiple imputation is preferable to
  dropping cases, but the strata must be defined consistently across imputations.
...
- **`brms`** -- Bayesian; preferred when cells are sparse or dimensions have few
  levels (regularising priors, honest intervals).
- **`wemix`** -- design-weighted fits for **complex survey data**
  (`sampling_weights =`); see [Design-weighted MAIHDA](design_weighted.html).
...
3. [Finding the interactions that matter](finding_interactions.html).
...
5. Specialised designs: [binary outcomes](binary_outcomes.html),
   ...
   [survey weights](design_weighted.html), and
   [Bayesian / sparse](bayesian_sparse_maihda.html).
