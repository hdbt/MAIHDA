...
The package ships `maihda_discriminatory_accuracy()`, which bundles the two
individual-level summaries for a logistic MAIHDA model: the **AUC / C-statistic**
(how well the predicted probabilities separate cases from non-cases) and the
**Median Odds Ratio (MOR)** (the between-stratum heterogeneity expressed on the
odds-ratio scale). The strata-only model's AUC is the discriminatory accuracy of
the intersectional strata *themselves* -- Merlo's central quantity. Comparing it
with the adjusted model shows whether individual information beyond stratum
