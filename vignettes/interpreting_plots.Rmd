...
One limit worth knowing: a few accent colours are set directly inside the geoms
(for example the blue points in the `predicted` view), not mapped through a
scale, so adding a `scale_colour_*()` will not recolour them. Themes, labels,
palettes on the mapped fills, and extra layers are the parts you can adjust
without editing the package internals.
