...
`WeMix::mix()` -- weighted pseudo-maximum-likelihood -- and `engine = "lme4"` with
sampling weights is an error rather than a silent misfit. The whole workflow
...
The wemix engine covers the canonical `gaussian(identity)` / `binomial(logit)`
...
likelihood weights (a *pseudo-posterior*: design-consistent point estimates, but
