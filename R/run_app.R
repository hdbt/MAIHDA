...
maihda_app_generate_code <- function(outcome_var, grouping_vars,
  ...
  lines <- c(
    ...
    "",
    "# 3. Build intersectional strata from the grouping variables",
    ...
    "",
    "# 4. Fit the null and adjusted MAIHDA models",
    sprintf('null_model     <- fit_maihda(%s, data = data, family = "%s")',
            null_fmla, family),
    sprintf('adjusted_model <- fit_maihda(%s, data = data, family = "%s")',
            adj_fmla, family),
    "",
    "# 5. Variance partition coefficient, fixed effects and stratum estimates",
    "summary(adjusted_model)",
    "",
    "# 6. Proportional change in variance (PCV)"
  )
