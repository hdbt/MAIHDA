...
# The prior-weights rule itself, over a response `y` and prior weights `w` ALREADY
# aligned to the same rows: the aggregated success / trial counts they imply, or NULL
# when they are not an aggregated binomial. Factored out of the fitted-model wrapper
# above so the PRE-FIT maihda_describe(weights = ) path applies the identical rule --
# describe, the AUC, and the fitted-model and formula spellings of describe must all
# agree about whether a given (y, w) pair is an aggregated binomial. `forced` is the
# wrapper's binomial_weights = "trials"; the pre-fit path has no such override and
# always takes the "auto" rule.
maihda_agg_counts_from_weights <- function(y, w, forced = FALSE,
                                           context = "Aggregated-binomial AUC") {
  ...
  list(successes = maihda_da_proportion_successes(y, trials, context = context),
       trials = trials)
}
