...
The MAIHDA package provides tools for conducting Multilevel Analysis of
Individual Heterogeneity and Discriminatory Accuracy (MAIHDA). This
...
effects of multiple social categories.
...
The summary provides variance partition coefficients and
...
For Bayesian inference, you can use the brms engine (requires brms
package):
...
The VPC indicates the proportion of variance in the outcome that is
attributable to differences between strata (intersectional categories).
A higher VPC suggests greater heterogeneity between strata.
...
The random effects represent deviations from the overall mean for each
stratum, after adjusting for fixed effects. Positive values indicate
higher outcomes in that stratum; negative values indicate lower
outcomes.
