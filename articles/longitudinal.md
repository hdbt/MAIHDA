...
- **Time coding.** You can code time however is natural – waves 0, 1, 2,
  …, waves 10, 11, …, age, or calendar year. When the axis does not
  start at 0, the growth terms are fit on internally *centered* time
  (`time - min(time)`, with a message), because the raw polynomial basis
  over an offset range is ill-conditioned and can silently converge to a
