...
* `maihda_describe()` now reads the trial counts of an aggregated binomial written the way `?glm` documents -- a proportion response with the denominators passed as `weights =` -- which it previously took to be 1 per row: a 12-row, 340-of-617 sample was reported as "6.420795 events / 12 trials (53.5%)" where the `cbind(successes, failures)` spelling of the same data gave "340 events / 617 trials (55.1%)". It defers to the detector `maihda_discriminatory_accuracy()` already uses, so the two cannot report different sample sizes for one model; a 0/1 response carrying integral weights stays a precision-weighted Bernoulli for both.
