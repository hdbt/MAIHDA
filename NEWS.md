...
* `summary(df_method = "bootstrap")` refers lme4 fixed-effect p-values and intervals to a null-restricted parametric bootstrap, the reference to use for a GLMM, whose Wald z is anticonservative for terms constant within a stratum. It costs `n_boot` refits per fixed-effect term, and the intercept, having no null model to simulate from, is `NA`. `tidy(component = "fixed")` carries it through.
