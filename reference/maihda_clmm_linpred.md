...
`newdata`) and multiplied by the location coefficients `beta` (a clmm
has *no* intercept column – it is absorbed by the thresholds – so
`beta`'s names select the right columns), any formula offset term is
evaluated on `newdata` and added (so an offset-only null model still
predicts its offset), and `include_re` adds each row's stratum
conditional mode (an unseen stratum contributes 0 – the zero-effect
fallback that
