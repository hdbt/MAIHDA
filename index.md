...
# 1 & 2. Create intersectional strata & Fit MAIHDA model
# You can directly specify the stratifying variables in the random effects
...
# 5. Visualize results (New Shiny App Visualizations)
# Plot stratum-level predictions (Caterpillar plot)
...
# Risk vs Effect size
plot_maihda(model, type = "risk_vs_effect")

# Additive vs Intersectional effect decomposition
plot_maihda(model, type = "effect_decomp")

# Ternary plot separating baseline, additive, and interaction variance
out_ternary <- maihda_ternary_plot(model)
print(out_ternary$plot)

# Deviance analysis for predictions
dev_panels <- plot_prediction_deviation_panels(model$model)
print(dev_panels)
```
...
- **Predicted**: Caterpillar plots of stratum random effects with
