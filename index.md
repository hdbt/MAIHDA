...
[![Lifecycle:
stable](https://img.shields.io/badge/lifecycle-stable-green.svg)](https://lifecycle.r-lib.org/articles/figures/lifecycle-stable.svg)
...
The generics come from the lightweight `generics` package (the same ones
`broom` re-exports), so
[`tidy()`](https://generics.r-lib.org/reference/tidy.html)/[`glance()`](https://generics.r-lib.org/reference/glance.html)
work whether you load `broom`, `generics`, or just `MAIHDA`. The layout
is uniform across the lme4, brms, WeMix, and ordinal engines. Note that
`tidymodels` (parsnip/workflows) does not fit `lme4` random-effects
models, so this is the tidy-*data* convention, not a modeling-pipeline
integration.
